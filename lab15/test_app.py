...
def test_form(client, register_user, login_user):
    with client:
        response = client.get(f'/candidate/form', follow_redirects=True)
        
        assert b'Candidate Name' in response.data
        assert b'Phone Number' in response.data


def test_db_form_table(client, register_user, login_user):
    with client:
        response = client.get(f'/candidate/db_form_table', follow_redirects=True)
